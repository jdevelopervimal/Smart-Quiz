            ...
            $.ajax({
                ...
                success: function (data) {
                    if (confirm('Are you ready to start?')) {
                        ...
                        $('#quiz-name').text(ans.quiz_name);
                        var time = parseInt(ans.quiz_duration, 10);
                        $('.removable').remove();
                        if(count > 1 && time > 0){
                            plotSideBar(bigdata, ans.attempts_qids,'enable');
                            plotAnswersheet(bigdata, ans.attempts_oids);
                            accessFullQuiz(bigdata, parseInt(ans.current[0], 10), parseInt(ans.current[1], 10),time,1);
                        }else{
                            plotSideBar(bigdata, ans.attempts_qids);
                            plotAnswersheet(bigdata, ans.attempts_oids,'disable');
...
//        });
    ...
    function accessFullQuiz(data, category, cur_ques,time,attempt) {
        $('#prev-btn').unbind('click');
        $('#skip-btn').unbind('click');
        $('#save-btn').unbind('click');
        $('#next-btn').unbind('click');
        ...
        if (category < count) {
            ...
            function timeisUp() {
                ...
                $('#save-btn').unbind('click');
                $('#next-btn').unbind('click');
                accessFullQuiz(data, category_count, 1,time,attempt);
            }
            ...
            $('#next-btn').click(function () {
                ...
                var cur_qid = (child > 0) ? child : questions[current - 1].qid;
//                var ar ='checkTextarea(cur_qid) : '+checkTextarea(cur_qid);
//                ar += '  checkCheckboxes(cur_qid) : '+checkCheckboxes(cur_qid);
//                ar += '  checkRadios(cur_qid) : '+checkRadios(cur_qid);
//                ar += '  cur_qid : '+questions[current - 1].qid;
//                ar += '  ploting : '+ploting;
//                
//                alert(ar);
                if (checkTextarea(cur_qid) || checkCheckboxes(cur_qid) || checkRadios(cur_qid)) {
            ...
            $('li.switch-question-single').click(function () {
                ...
                if ((qno != '') && (qno != 'undefined')) {
                    $('.removable').remove();
                    if(cat != category){
                        accessFullQuiz(data, cat, qno,time,attempt);
                    }else{
                        question(questions[qno - 1], (qno));
                        $('#next-btn').data('current', (qno));    
                    }
    ...
    function plotSideBar(category, qids,status) {
        var arrayQids = qids.split(',')
        var cat_switch = '';
        var question_switch = '';
        var disable = (status == 'disable') ? 'disabled' : '';
        for (var i = 0; i < category.length; i++) {
            cat_switch += '<li role="presentation" id="catab_' + i + '" class="' + ((i == 0) ? 'active' : disable) + '"><a href="#cat_' + i + '" id="catl_' + i + '" aria-controls="cat_' + i + '" role="tab" data-toggle="' + (((i == 0) || status == 'enable') ? 'tab' : '') + '">' + category[i].category_name + '</a></li>';
            question_switch += '<div role="tabpanel" class="tab-pane ' + ((i == 0) ? 'active' : disable) + '" id="cat_' + i + '">';
            ...
            if (quest.length > 0) {
                for (var j = 0; j < quest.length; j++) {
                    ...
                    question_switch += '<li class="switch-question-single" data-cat="'+i+'" id="list-ques_' + i + '_' + (j + 1) + '" data-queno="' + (j + 1) + '" data-queid="' + quest[j].qid + '"> <a class="' + checked + '">' + (j + 1) + '</a></li>';
                }
            } else {
