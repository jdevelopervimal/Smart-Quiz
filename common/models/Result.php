...
//            [['score_ind'], 'string', 'max' => 2000]
    ...
    public static function setResultEnvironment($user_id = '', $quiz_id = '', $max_score)
    {
    ...
    public static function getUserResult($user_id = '')
    {
    ...
    public static function getAllResult($user_id = '')
    {
        if ($user_id == 1) {
            $connection = \Yii::$app->db;
...
//            . ' left join user u on		u.id = qr.uid'
//            . ' left join user_group ug on		ug.gid = u.group_id'
//            . ' where  qr.status = "1" AND u.id = '.$user_id;
//        $command = $connection->createCommand($sql);
//        $data = $command->queryAll();
//        return $data;
//    }
    ...
    {
        $connection = \Yii::$app->db;
        $sql = 'select qu.quiz_code,qr.percentage from quiz_result qr '
            . ' left join quiz qu on qu.quid = qr.quid'
            . ' where  qr.status = "1" AND qr.uid = 7 order by qr.rid DESC limit 50 ';//.$user_id;
        $command = $connection->createCommand($sql);
        $data = $command->queryAll();
        return $data;
    }
