...
class Question extends \yii\db\ActiveRecord {
    ...
    public static function getQuestionByCat($cat_id) {
        $connection = \Yii::$app->db;
    ...
    public static function getAllQuestions($request = '', $ids = array()) {
        $questions = array();
        if ($request == 'cat') {
    ...
    public static function getAllQuestionsSummery($request = '', $ids = array()) {
        $questions = array();
        if ($request == 'cat') {
            $questions = Question::find()->select('qid,question,did,q_type,description')->where(['cid' => $ids])->andWhere(['parent_qid' => 0])->asArray()->all();
        } else {
            $questions = Question::find()->select('qid,question,did,q_type,description')->where('qid IN(' . implode(',', $ids) . ')')->andWhere(['parent_qid' => 0])->asArray()->all();
        }
        foreach ($questions as $key => $qd) {
            if ($qd['q_type'] == 3) {
                $sub_ques = Question::find()->select('qid,question,did,q_type,description')->where(['parent_qid' => $qd['qid']])->asArray()->all();
                $options = null;
                foreach ($sub_ques as $key2 => $sub_que) {
                    $sub_ques[$key2]['options'] = Options::getAllOptions($sub_que['qid']);
                }
    ...
    public static function getQuestionCountByIds($qids = array()) {
        $connection = \Yii::$app->db;
    ...
    public static function getQuestionCountByCategory($catids = array()) {
        $connection = \Yii::$app->db;
        $sql = 'SELECT count(q.qid) as noofques FROM qbank q'
                . ' LEFT JOIN qbank qp on qp.parent_qid = q.qid'
